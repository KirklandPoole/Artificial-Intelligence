...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        '''
        # Adds action A level to the planning graph as described in the Russell-Norvig text
        ...
        # create list to hold A nodes to be added to the current level
        list_of_nodes = []
        for current_action in self.all_actions:
            pgNode_a_action = PgNode_a(current_action)
            # 1. determine what actions to add and create those PgNode_a objects
            if pgNode_a_action.prenodes.issubset(self.s_levels[level]):
                list_of_nodes.append(pgNode_a_action)
                for node_level in self.s_levels[level]:
                    # 2. connect the nodes to the previous S literal level
                    node_level.children.add(pgNode_a_action)
                    pgNode_a_action.parents.add(node_level)
    ...
    def add_literal_level(self, level):
        ...
        setOfNodes = set()
        # 1. determine what literals to add
        for aNodeFromPreviousLevel in self.a_levels[level - 1]:
            for sNode in aNodeFromPreviousLevel.effnodes:
                # 2. connect the nodes
                setOfNodes.add(sNode)
                sNode.parents.add(aNodeFromPreviousLevel)
                aNodeFromPreviousLevel.children.add(sNode)
        # Add S nodes to the current level in self.s_levels[level]
    ...
    def update_a_mutex(self, nodeset):
        ...
        for interation, node1 in enumerate(nodelist[:-1]):
            for node2 in nodelist[interation + 1:]:
                # Serialized Actions OR Inconsistent Effects OR Interference OR Competing needs
                if (self.serialize_actions(node1, node2) or self.inconsistent_effects_mutex(node1, node2) or self.interference_mutex(node1, node2) or self.competing_needs_mutex(node1, node2)):
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # action negates an effect flag
        actionNegatesEffect = False
        # The Action instance associated with an action node is accessible through the PgNode_a.action attribute.
        for effectInNodeA1 in node_a1.action.effect_add:
            if effectInNodeA1 in node_a2.action.effect_rem:
                # action negates an effect
                actionNegatesEffect = True
                return actionNegatesEffect

        # The Action instance associated with an action node is accessible through the PgNode_a.action attribute.
        for effectInNodeA2 in node_a2.action.effect_add:
            if effectInNodeA2 in node_a1.action.effect_rem:
                # action negates an effect
                actionNegatesEffect = True
                return actionNegatesEffect
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for nodeA1Effect in node_a1.action.effect_add:
            if nodeA1Effect in node_a2.action.precond_neg:
                return True
        for nodeA2Effect in node_a2.action.effect_add:
            if nodeA2Effect in node_a1.action.precond_neg:
                return True
        for nodeA1Effect in node_a1.action.effect_rem:
            if nodeA1Effect in node_a2.action.precond_pos:
                return True
        for nodeA2Effect in node_a2.action.effect_rem:
            if nodeA2Effect in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_a1_precondition in node_a1.parents:
            for node_a2_precondition in node_a2.parents:
                if node_a2_precondition.is_mutex(node_a1_precondition):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        inconsistent_support_mutex_test = True
        for node_s1_precondition in node_s1.parents:
            for node_s2_precondition in node_s2.parents:
                if not node_s1_precondition.is_mutex(node_s2_precondition):
                    return False
        return inconsistent_support_mutex_test
    ...
    def h_levelsum(self) -> int:
        ...
        # Reference : Understand Level-sum heuristic
        # https://discussions.udacity.com/t/understand-level-sum-heuristic/225706/3
        #

        #
        # for each goal in the problem, determine the level cost, then add them together
        #
        for problemGoal in self.problem.goal:
            hasProblemGoalBeenFound = False
            for levelNumber in range(len(self.s_levels)):
                for sLevel in self.s_levels[levelNumber]:
                    if not hasProblemGoalBeenFound:
                        if problemGoal == sLevel.literal:
                            hasProblemGoalBeenFound = True
                            # add the level number to the level_sum
                            level_sum = level_sum +  levelNumber
        return level_sum
